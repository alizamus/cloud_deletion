...
def main(argv):
	...
	for tenant in tenants:
		res= os.system("/root/cloud_deletion/code/config --username admin --password " + admin_pass + " --tenant " + tenant +  " --api-server 127.0.0.1 show policy" + " > " + "/root/cloud_deletion/test.csv")
		...
		with open('/root/cloud_deletion/test.csv', 'rb') as csvfile:
			...
			for row in rules:
				...
				b = a.split('    ',1)[1]
				c = a.split('    ',1)[0]
				comand = "/root/cloud_deletion/code/config --username admin --password " + admin_pass + " --tenant " + tenant +  " --api-server 127.0.0.1 delete policy " + "'" + b + "'"
				#print comand	
				os.system(comand)
